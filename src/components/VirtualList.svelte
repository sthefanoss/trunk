...
const handleHeightChangesScrollCorrection = (
  ...
  if (isScrolling) {
    ...
    if (pending !== 0) {
      ...
      const prev = (heightManager.viewport as unknown as Record<string, number>)[key as string] as
        | number
        | undefined;
  ...
  if (Math.abs(heightChangeAboveViewport) > 2) {
    const newScrollTop = clampValue(currentScrollTop + heightChangeAboveViewport, 0, maxScrollTop);
    ...
    (result) => {
      ...
      if (heightManager.isReady && heightManager.initialized) {
        ...
        if (Math.abs(deltaTotal) > 1) {
          ...
          if (isAtBottom) {
            const adjusted = clampValue(currentScrollTop + deltaTotal, 0, maxScrollTop);
