...
- truth: "Shift+click range selection works without triggering browser text selection"
  ...
  severity: minor
  test: 2
  root_cause: "e.preventDefault() is called during the click event, but browser text selection on shift+click starts during mousedown which fires before click. By the time click handler runs, text selection has already occurred."
  artifacts:
    - path: "src/components/DiffPanel.svelte"
      issue: "onclick handler at line 557 calls preventDefault too late; needs onmousedown with shiftKey guard"
  missing:
    - "Add onmousedown handler that calls e.preventDefault() when e.shiftKey is true on selectable diff line divs"
  debug_session: ".planning/debug/shift-click-text-selection.md"
