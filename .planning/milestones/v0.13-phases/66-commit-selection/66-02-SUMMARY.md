...
**Verifier note on the `grep -n "sessions.0.lock" … >= 3` acceptance criterion:** the lock now lives inside `mutate_session_rmw`, not at the three mutating command call sites, so a mechanical grep at those sites returns fewer hits than the literal count suggests. The criterion's own hedge says "verify by reading the bodies, not just grep." The truth it guards (RMW serialized under the `ReviewSessionsState` mutex; two rapid clicks cannot lose a write) is satisfied and proven by `selection_rmw_serialized`. The code was deliberately NOT changed to satisfy the grep — the test is the real check.
